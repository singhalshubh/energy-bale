...
int main(int argc, char **argv) {
    ...
    // --- PAPI SETUP ONLY ON PE0 (you can restrict with if (mype == 0) if desired) ---
    int papi_ok   = 1;
    ...
    // -------- 100 ATOMIC FETCH_INC OPERATIONS --------
    for (int i = 0; i < iters; i++) {
        if (mype != target_pe) {
            long old = shmem_long_atomic_fetch_inc(x, target_pe);
            shmem_quiet();
        }
    }
    t1 = now_sec();
    ...
    *energy_per_op = 0.0;
    ...
    if(mype == 0)
        fprintf(stderr, "atomic_fetch_inc: time_per_op=%e s, total energy=%e J\n",
